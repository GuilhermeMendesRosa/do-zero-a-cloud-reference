...
import { Cloud, LayoutDashboard, Loader2, MoreHorizontal, Pencil, Plus, RefreshCw, Trash2, WifiOff } from "lucide-react"
...
export default function App() {
  ...
  return <div className="flex min-h-dvh bg-white text-neutral-950">
      ...
      <div className="ml-auto flex min-w-0 items-center gap-1 overflow-x-auto md:ml-0 md:block md:overflow-visible">
        ...
        <nav className="flex gap-1 md:block md:space-y-1" aria-label="Quadros">
          {boards.map((board) => <div key={board.id} className={`group flex shrink-0 items-center rounded-lg ${selectedId === board.id ? "bg-neutral-100" : "hover:bg-neutral-50"}`}>
            <button disabled={interactionBusy} className="max-w-40 flex-1 truncate px-3 py-2 text-left text-sm font-medium disabled:cursor-not-allowed disabled:opacity-50 md:max-w-none" onClick={() => setSelectedId(board.id)}>{board.name}</button>
    ...
    <main className="flex min-h-dvh min-w-0 flex-1 flex-col pt-16 md:pt-0">
      <header className="flex h-20 shrink-0 items-center justify-between px-6 md:px-8"><div className="min-w-0"><p className="text-xs font-medium uppercase tracking-widest text-neutral-400">Meu quadro</p><h1 className="truncate text-xl font-semibold md:text-2xl">{selectedBoard?.name ?? "Seus projetos"}</h1></div><div className="flex items-center gap-3">{requestInFlight && <span className="inline-flex items-center gap-1.5 text-xs font-medium text-neutral-500" role="status"><Loader2 className="h-4 w-4 animate-spin" />Salvando…</span>}{selectedBoard && <Button variant="outline" size="sm" onClick={() => { void boardQuery.refetch() }} disabled={interactionBusy}><RefreshCw className={`h-4 w-4 ${boardQuery.isFetching ? "animate-spin" : ""}`} />{boardQuery.isFetching ? "Atualizando…" : "Atualizar"}</Button>}</div></header>
